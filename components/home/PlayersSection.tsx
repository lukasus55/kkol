    ...
    useEffect(() => {
        async function load() {
            try {
                ...
                if (!rankRes.ok || !tourRes.ok) {
                    throw new Error(`API fetch failed: rank=${rankRes.status}, tour=${tourRes.status}`);
                }
                ...
                const tournaments = (tournamentsData && typeof tournamentsData === 'object')
                    ? (Object.values(tournamentsData) as Tournament[])
                    : [];

                const mapped = targetPlayers.map(id => {
                    const rankData = rankingList.find((r: any) => r && r.id === id) || { ranking: 0, name: id, pfpSrc: '/img/default_pfp.webp' };

                    let sTierWins = 0;
                    tournaments.forEach(t => {
                        if (t && t.finished && t.details?.tier === 'S' && t.standings) {
                            const topPos = t.standings.length > 0 ? t.standings[0].position : null;
                            if (topPos === 1) {
                                const winners = t.standings.filter(s => s && s.position === 1);
                                if (winners.some(w => w && w.id === id)) {
                                    sTierWins++;
                                }
                            }
                        }
                    ...
                    return {
                        id,
                        name: rankData.name || id,
                        pfpSrc: rankData.pfpSrc || '/img/default_pfp.webp',
                        ranking: Number(rankData.ranking) || 0,
                        sTierWins,
    ...
    return (
        <section className="w-full flex flex-col items-center">
            <div className="w-full max-w-[1016px] px-8 sm:px-12 flex flex-col items-start">
                <h2 className="mb-3 sm:mb-4 text-2xl sm:text-3xl font-bold tracking-tight text-white">Wyróżnieni Gracze</h2>
                ...
                <div className="relative flex items-center w-full">
                    ...
                    <div
                        ...
                        className="flex gap-4 sm:gap-6 overflow-x-auto snap-x snap-mandatory scroll-smooth w-full py-2"
