...
export function TournamentEventsModal({ isOpen, onClose, tournament }: TournamentEventsModalProps) {
  ...
  return (
    ...
    >
      <div className="text-dashboard-text-s2 text-sm mb-4">
      ...
      <div className="flex flex-col gap-3 min-h-[150px] max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
        {loading ? (
          <div className="text-center text-dashboard-text-s2 py-8">Ładowanie wydarzeń...</div>
        ) : events.length === 0 ? (
          <div className="text-center text-dashboard-text-s2 py-8">Brak wydarzeń dla tego turnieju.</div>
        ) : (
          events
            ...
            .map(event => (
              <div 
                ...
                className={`flex items-center justify-between p-4 rounded-md border ${
                  event.is_major 
                    ? 'bg-dashboard-primary/10 border-dashboard-primary/30' 
                    : 'bg-dashboard-bg-s3 border-dashboard-stroke'
                }`}
              >
                ...
                <div className="text-dashboard-text-s2 text-xs font-medium">
